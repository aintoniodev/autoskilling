...
(def terminal-helpers
  ["terminal-app.sh" "iterm2.sh" "ghostty.sh" "windows-terminal.sh" "windows-native.sh" "none.sh"])
...
(defn start-handoff-daemon! [ctx]
  ...
  (let [daemon-script (str (fs/path (:script-dir ctx) "handoffd.bb"))
        ;; Windows: bb cannot exec a .bb file directly; run it via the bb runtime.
        daemon-command (if (str/includes? (System/getProperty "os.name") "Windows")
                         ["bb" daemon-script]
                         [daemon-script])
        command (into (vec (sleep-inhibitor-prefix))
                      (concat daemon-command [(str (:working-dir ctx))]))]
